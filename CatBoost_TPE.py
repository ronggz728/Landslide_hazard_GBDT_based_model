...
def percept(args):
    ...
    ppn = CatBoostClassifier(n_estimators = int(args["n_estimators"]),
                            max_depth=int(args['max_depth']),
                            learning_rate = args['learning_rate'],
                            subsample = args['subsample'],random_state= 0)
    ppn.fit(x_train, y_train)
    y_pred = ppn.predict(x_test)
    return -accuracy_score(y_test, y_pred)

from hyperopt import fmin,tpe,hp,partial,Trials
space = dict(n_estimators=hp.quniform("n_estimators", 50, 500, 1),
