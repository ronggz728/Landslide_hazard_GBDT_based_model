...
# max_depth： 树的最大深度。默认值是6，取值范围为：[1,∞]；
# eta：为了防止过拟合，更新过程中用到的收缩步长。在每次提升计算之后，算法会直接获得新特征的权重。eta通过缩减特征的权重使提升计算过程更加保守。默认值为0.3，取值范围为：[0,1]；
# silent：取0时表示打印出运行时信息，取1时表示以缄默方式运行，不打印运行时信息，默认值为0；
# objective： 定义学习任务及相应的学习目标，“binary:logistic” 表示二分类的逻辑回归问题，输出为概率。
##clf = xgb.XGBClassifier(n_estimators = 100, max_depth =6 ,eta= 1,eval_metric='mlogloss')
...
dataset_1 = pd.read_csv('F://fifth paper//输出统计//screened//XGBOOST//调序删表名版//input_1.csv', header=None, sep=',',
                        names=['fault', 'road', 'river', 'lithology', 'elevation', 'slope', 'NDVI', 'plan', 'geological', 'rain', 'SPI', 'TWI', 'TRI'])

x_columns_1 = [x for x in dataset_1.columns]
x_1 = dataset_1[x_columns_1]

dataset_2 = pd.read_csv('F://fifth paper//输出统计//screened//XGBOOST//调序删表名版//input_2.csv', header=None, sep=',',
                        names=['fault', 'road', 'river', 'lithology', 'elevation', 'slope', 'NDVI', 'plan', 'geological', 'rain', 'SPI', 'TWI', 'TRI'])

x_columns_2 = [x for x in dataset_2.columns]
x_2 = dataset_2[x_columns_2]

dataset_3 = pd.read_csv('F://fifth paper//输出统计//screened//XGBOOST//调序删表名版//input_3.csv', header=None, sep=',',
                        names=['fault', 'road', 'river', 'lithology', 'elevation', 'slope', 'NDVI', 'plan', 'geological', 'rain', 'SPI', 'TWI', 'TRI'])

x_columns_3 = [x for x in dataset_3.columns]
x_3 = dataset_3[x_columns_3]

dataset_4 = pd.read_csv('F://fifth paper//输出统计//screened//XGBOOST//调序删表名版//input_4.csv', header=None, sep=',',
                        names=['fault', 'road', 'river', 'lithology', 'elevation', 'slope', 'NDVI', 'plan', 'geological', 'rain', 'SPI', 'TWI', 'TRI'])

x_columns_4 = [x for x in dataset_4.columns]
x_4 = dataset_4[x_columns_4]

dataset_5 = pd.read_csv('F://fifth paper//输出统计//screened//XGBOOST//调序删表名版//input_5.csv', header=None, sep=',',
                        names=['fault', 'road', 'river', 'lithology', 'elevation', 'slope', 'NDVI', 'plan', 'geological', 'rain', 'SPI', 'TWI', 'TRI'])
...
exp_1.to_csv("F://fifth paper//OUTPUT//XGB//XGB_T_ALL_1.csv")
probability_2 = clf.predict_proba(x_2)
exp_2 = DataFrame(probability_2)
exp_2.to_csv("F://fifth paper//OUTPUT//XGB//XGB_T_ALL_2.csv")
probability_3 = clf.predict_proba(x_3)
exp_3 = DataFrame(probability_3)
exp_3.to_csv("F://fifth paper//OUTPUT//XGB//XGB_T_ALL_3.csv")
probability_4 = clf.predict_proba(x_4)
exp_4 = DataFrame(probability_4)
exp_4.to_csv("F://fifth paper//OUTPUT//XGB//XGB_T_ALL_4.csv")
probability_5 = clf.predict_proba(x_5)
exp_5 = DataFrame(probability_5)
exp_5.to_csv("F://fifth paper//OUTPUT//XGB//XGB_T_ALL_5.csv")
