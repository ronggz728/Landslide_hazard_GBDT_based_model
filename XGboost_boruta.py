...
feat_selector = BorutaPy(estimator=clf, n_estimators=100, perc=50, verbose=2, random_state=1)
feat_selector.fit(x_train.values, y_train.values)

# 获取筛选后的特征
selected_features = [feature for feature, selected in zip(x_columns_train, feat_selector.support_) if selected]
# 使用筛选后的特征索引来获取筛选后的特征矩阵
X_filtered_train = x_train.values[:, x_train.columns.get_indexer(selected_features)]
X_filtered_test = x_test.values[:, x_test.columns.get_indexer(selected_features)]
X_filtered = X.values[:, X.columns.get_indexer(selected_features)]

# 使用筛选后的特征训练模型
clf1.fit(X_filtered_train, y_train.values)

# 使用训练好的模型进行预测
predict_target = clf1.predict(X_filtered_test)

probability_test = clf1.predict_proba(X_filtered_test)
exp_test = DataFrame(probability_test)
exp_test.to_csv("F://fifth paper//OUTPUT//XGB//XGB_bor_test.csv")
print(probability_test)

probability_train = clf1.predict_proba(X_filtered_train)
exp_train = DataFrame(probability_train)
exp_train.to_csv("F://fifth paper//OUTPUT//XGB//XGB_bor_train.csv")
